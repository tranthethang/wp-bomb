...
const AutoAttachThumbnail = () => {
    ...
    const fetchStats = async () => {
        try {
            const data = await apiFetch({ path: '/wpbomb/v1/auto-attach-thumbnail/stats' });
            if (data.success) {
    ...
    const handleExecute = async (e) => {
        ...
        if (!confirm(__('Are you sure you want to attach thumbnails? This will set thumbnails for posts based on the specified ID range.', 'wp-bomb'))) {
            return;
        }
        ...
        try {
            ...
            setNotice({
                type: 'error',
                message: error.message || __('An error occurred during execution.', 'wp-bomb')
            });
        } finally {
            setIsLoading(false);
        }
        ...
        <div className="wp-bomb-module mb-8">
            <div className="mb-6">
                ...
                <p className="text-[13px] text-wp-sub leading-relaxed max-w-4xl">
                    {__('This module scans posts within the specified ID range and automatically attaches the first image found in the content as the featured image (thumbnail) if one is not already set.', 'wp-bomb')}
                </p>
            </div>
