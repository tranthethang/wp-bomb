...
const RegenerateThumbnails = () => {
    ...
    const handleStart = async () => {
        ...
        try {
            ...
            if (response.success) {
                ...
                processBatches(ids, 0, { completed: 0, failed: 0 }, initialStats.startTime);
            }
        } catch (error) {
            console.error('Failed to start process', error);
            setStatus('error');
            setNotice({ type: 'error', message: __('Failed to start the process.', 'wp-bomb') });
        }
    };
    ...
    const processBatches = async (allIds, currentIndex, currentStats, startTime) => {
        ...
        try {
            ...
            processBatches(allIds, processed, { completed: newCompleted, failed: newFailed }, startTime);
        } catch (error) {
            console.error('Batch failed', error);
            setStatus('error');
            setNotice({ type: 'error', message: __('Batch processing failed.', 'wp-bomb') });
        }
    };
            ...
            <div className="divide-y divide-gray-100">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 items-start">
                    <div className="md:col-span-1">
                        <label className="text-sm font-semibold text-wp-heading">{__('Images per Batch', 'wp-bomb')}</label>
                        <p className="text-xs text-gray-500 mt-1 leading-relaxed">{__('The number of images processed in a single AJAX request.', 'wp-bomb')}</p>
                    </div>
                    <div className="md:col-span-2">
                        <RangeControl
                            ...
                            min={1}
                            max={50}
                            help={__('Low (5-10) for shared hosting, High (20+) for dedicated servers.', 'wp-bomb')}
                        />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 items-start">
                    <div className="md:col-span-1">
                        <label className="text-sm font-semibold text-wp-heading">{__('Target Thumbnail Sizes', 'wp-bomb')}</label>
                        <p className="text-xs text-gray-500 mt-1 leading-relaxed">{__('Select specific sizes to regenerate to save time and resources.', 'wp-bomb')}</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 items-start">
                    <div className="md:col-span-1">
                        <label className="text-sm font-semibold text-wp-heading">{__('Advanced Options', 'wp-bomb')}</label>
                    </div>
                    <div className="md:col-span-2">
                        <CheckboxControl
                            label={__('Skip existing correctly sized thumbnails', 'wp-bomb')}
                            help={__('Only regenerate if the file is missing or dimensions are incorrect.', 'wp-bomb')}
                            checked={settings.skipExisting}
                            onChange={(checked) => setSettings({ ...settings, skipExisting: checked })}
                        />
                    </div>
                </div>
            </div>
        ...
        return (
            <Card className="bg-wp-surface border border-wp-border shadow-wp-card sm:rounded-sm">
                <CardHeader className="px-4 py-4 sm:px-6 border-b border-wp-border flex justify-between items-center bg-white">
                    <h3 className="text-sm font-semibold text-gray-900">{__('Progress', 'wp-bomb')}</h3>
                ...
                <CardBody className="p-4 sm:p-6 space-y-6">
                    <div className="space-y-2">
                        <div className="flex justify-between text-sm text-wp-sub mb-1">
                            <span>{__('Processing image ', 'wp-bomb')} <strong>{stats.processed}</strong> {__(' of ', 'wp-bomb')} <strong>{stats.total}</strong></span>
                        ...
                        <ProgressBar
                            ...
                            className="w-full"
                        />
                        <p className="text-xs text-wp-sub italic mt-1">{__('Last processed: ', 'wp-bomb')} <span className="text-gray-600">{lastProcessed || __('Waiting...', 'wp-bomb')}</span></p>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-2">
                        <div className="flex flex-col border-l-4 border-green-500 bg-white pl-4 py-1">
                            <span className="text-xs font-medium text-wp-sub uppercase tracking-wide">{__('Completed', 'wp-bomb')}</span>
                            <div className="flex items-baseline mt-1">
                                <span className="text-2xl font-semibold text-gray-900">{stats.completed}</span>
                                <span className="ml-2 text-sm text-gray-500">{__('items', 'wp-bomb')}</span>
                            </div>
                        </div>
                        <div className="flex flex-col border-l-4 border-red-500 bg-white pl-4 py-1">
                            <span className="text-xs font-medium text-wp-sub uppercase tracking-wide">{__('Failed', 'wp-bomb')}</span>
                            <div className="flex items-baseline mt-1">
                                <span className="text-2xl font-semibold text-gray-900">{stats.failed}</span>
                                <span className="ml-2 text-sm text-gray-500">{__('items', 'wp-bomb')}</span>
                            </div>
                        </div>
                        <div className="flex flex-col border-l-4 border-orange-500 bg-white pl-4 py-1">
                            <span className="text-xs font-medium text-wp-sub uppercase tracking-wide">{__('Pending', 'wp-bomb')}</span>
                            <div className="flex items-baseline mt-1">
                    ...
                    {isCompleted && (
                        <Notice status="success" isDismissible={false}>
                            {sprintf(__('Successfully regenerated thumbnails for %d images in %d seconds.', 'wp-bomb'), stats.completed, stats.duration)}
                        </Notice>
                    )}

                    {notice && <Notice status={notice.type} onRemove={() => setNotice(null)}>{notice.message}</Notice>}
                </CardBody>
                <CardFooter className="pt-4 border-t border-gray-100 flex flex-col sm:flex-row gap-3">
                    {isCompleted && (
                        <Button variant="primary" onClick={() => window.location.href = window.wpBombData.admin_url + 'upload.php'}>
    ...
    return (
        <div className="wp-bomb-module mt-8">
            <div className="mb-6">
                <h2 className="text-2xl font-medium text-gray-800 mb-2">{__('Regenerate Thumbnails', 'wp-bomb')}</h2>
                <p className="text-[13px] text-wp-sub leading-relaxed max-w-4xl">
                    {__('This tool regenerates thumbnails for all image attachments. This is useful if you have changed your theme or one of your thumbnail dimensions.', 'wp-bomb')}
                </p>
            </div>
